...
def customcnn(cnn_in, is_training, drop_rate):
    ...
    net = tf.reshape(cnn_in, (-1,7,7,16))
    net = tf.compat.v1.layers.conv2d(inputs=net, filters=128, kernel_size=(7,7))
    net = tf.compat.v1.layers.flatten(net)
    net = tf.compat.v1.layers.dense(net, 256)
    logits = tf.compat.v1.layers.dense(net, 10)
